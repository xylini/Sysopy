...
Charray *create_char_block(Charray *array, int index, char *block_content, int is_static) {
    ...
    int block_length = (int)strlen(block_content);
...
Charray *delete_char_block(Charray *array, int index, int is_static) {
    ...
    if (is_static == 0) free(array[index].char_block);
    if (array[index].char_block != NULL) array[index].char_block[0] = '\0';
    array[index].block_length = 0;
    array[index].block_sum = (int)NULL;
    array[index].char_block = NULL;
    return array;
...
Charray *change_char_block(Charray *charray, int index, char *new_block_content, int is_static) {
    if (is_static != 1 && is_static != 0) {
        fprintf(stderr, "change_char_block: is_static could be 1(is) or 0(is not)");
        exit(EXIT_FAILURE);
    }
    if(is_static == 0) free(charray[index].char_block);

    int nwblk_ln = strlen(new_block_content);
    if(is_static == 0) charray[index].char_block = calloc(nwblk_ln+1, sizeof(char));
    else charray[index].char_block = char_table[index];

    strcpy(charray[index].char_block, new_block_content);
    charray[index].char_block[nwblk_ln] = '\0';
    charray[index].block_length = nwblk_ln;
    charray[index].block_sum = sum_char_block(charray[index].char_block);
    return charray;
}
